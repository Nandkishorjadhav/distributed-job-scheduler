...
- **Root Causes & Solutions**:
  1. **Worker Process Not Running**:
     - _Fix_: Start the worker engine in a terminal: `npm run dev --prefix backend/worker`.
  2. **Queue is Paused**:
     - _Fix_: Check the queue status in **[Queues](http://localhost:5173/queues)**. If `PAUSED`, click the **Resume** ($\blacktriangleright$) button.
  3. **Worker Concurrency Saturated**:
     - _Fix_: Check the **[Workers](http://localhost:5173/workers)** page. If all concurrency slots are in use (`BUSY`), increase worker concurrency in `.env` (`WORKER_CONCURRENCY=10`) or start additional worker nodes.
